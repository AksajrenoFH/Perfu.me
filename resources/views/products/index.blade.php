                ...
                <form action="{{ route('products.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    ...
                    <div class="md:col-span-5">
                        ...
                        <div class="relative">
                            ...
                            <input id="product-search" type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama parfum..."
                    ...
                    <div class="md:col-span-3 flex gap-2">
                        ...
                        @if(request('type') || request('search'))
                            <a href="{{ route('products.index') }}" class="px-4 py-2.5 rounded-2xl bg-gray-100 text-xs font-bold text-gray-600 hover:bg-gray-200 transition text-center flex items-center justify-center">Reset</a>
                        @endif
                    </div>
                </form>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-100 text-gray-700">
                            @forelse ($products as $index => $product)
                                <tr class="hover:bg-gray-50/70 transition-colors">
                                    <td class="py-4 px-6 text-center font-bold text-gray-400">{{ $products->firstItem() + $index }}</td>
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-xs text-gray-400 mt-0.5">Gender: <span class="text-gray-600 font-medium">{{ $product->gender ?: 'Universal' }}</span></div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-xs">
                                        <div class="flex flex-col items-start gap-1.5">
                                            ...
                                            @else
                                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-600">{{ $product->category ?: '-' }}</span>
                                            @endif
                                            <span class="text-gray-400 font-medium">{{ $product->variant ?: '-' }}</span>
                                        </div>
                                    </td>

                                    <td class="py-4 px-6 font-black text-gray-900 text-sm whitespace-nowrap">Rp {{ number_format($product->price, 0, ',', '.') }}</td>

                                    <td class="py-4 px-6 text-center">
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold {{ $product->stock < 10 ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200' }}">
                                            <span class="h-1.5 w-1.5 rounded-full bg-current"></span>
                                            {{ $product->stock }} pcs
                                        </span>
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-right">
                                        <div class="flex justify-end items-center gap-3">
                                            ...
                                            <button type="button" @click="deleteModalOpen = true; deleteUrl = '{{ route('products.destroy', $product->id) }}'; productName = '{{ $product->name }}'"
