<x-app-layout>
    <div x-data="{ deleteModalOpen: false, deleteUrl: '', productName: '' }" class="min-h-screen bg-[#F8F9FA] py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">

            <!-- Top Header & Action Banner -->
            <div
                class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl shadow-xs border border-gray-100">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1 ml-5">Kelola seluruh stok, harga, dan varian aroma parfum
                        Perfu.me dengan mudah.</p>
                </div>
            ...
            @if(session('success'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
                    class="bg-emerald-50 border border-emerald-200 p-4 rounded-2xl flex items-center justify-between text-emerald-800 shadow-xs"
                    role="alert">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-8 h-8 rounded-xl bg-emerald-500 text-white flex items-center justify-center font-bold">
                            ✓</div>
            ...
            <div class="bg-white rounded-3xl shadow-xs border border-gray-100 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr
                                class="bg-gray-50/70 border-b border-gray-100 text-[11px] font-black text-gray-400 uppercase tracking-wider">
                        ...
                        <tbody class="divide-y divide-gray-50 text-xs font-medium text-gray-600">
                            @forelse ($products as $index => $product)
                                <tr class="hover:bg-gray-50/60 transition-colors group">
                                    <!-- No -->
                                    <td
                                        class="py-4 px-6 text-center font-bold text-gray-400 group-hover:text-black transition-colors">
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="w-12 h-12 rounded-2xl overflow-hidden bg-gray-100 border border-gray-200/60 flex-shrink-0 shadow-xs group-hover:scale-105 transition-transform">
                                                @if($product->image)
                                                    <img src="{{ asset('storage/' . $product->image) }}"
                                                        alt="{{ $product->name }}" class="w-full h-full object-cover">
                                                @else
                                                    <div
                                                        class="w-full h-full flex items-center justify-center text-[10px] text-gray-400 font-bold">
                                                        No Pic</div>
                                                @endif
                                            </div>
                                            <div>
                                                ...
                                                <div class="text-[11px] text-gray-400 mt-0.5">Gender: <span
                                                        class="text-gray-700 font-semibold">{{ $product->gender ?: 'Universal' }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>

                                    <!-- Kategori (Original / Refill dengan Icon SVG) -->
                                    <td class="py-4 px-6">
                                        @if($product->category == 'Original')
                                            <span
                                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[11px] font-bold bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/20">
                                        ...
                                        @elseif($product->category == 'Refill')
                                            <span
                                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[11px] font-bold bg-black text-white">
                                        ...
                                        @else
                                            <span
                                                class="px-2.5 py-1 rounded-lg text-[11px] font-semibold bg-gray-100 text-gray-600">
                                                {{ $product->category ?: '-' }}
                                            </span>
                                        @endif
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        <span
                                            class="inline-block px-3 py-1 rounded-xl font-bold text-xs {{ $product->stock < 10 ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-gray-100/80 text-gray-800' }}">
                                            {{ $product->stock }} pcs
                                        </span>
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        @if($product->is_best_seller)
                                            <span
                                                class="inline-block bg-[#D4AF37] text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider shadow-xs">
                                                Best Seller
                                            </span>
                                        @else
                                    ...
                                    <td class="py-4 px-6 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('products.show', $product->id) }}"
                                                class="p-2 bg-gray-50 hover:bg-black hover:text-white text-gray-600 rounded-xl transition-all shadow-xs"
                                                title="Lihat">
                                            ...
                                            <a href="{{ route('products.edit', $product->id) }}"
                                                class="p-2 bg-[#D4AF37]/10 hover:bg-[#D4AF37] hover:text-white text-[#D4AF37] rounded-xl transition-all shadow-xs"
                                                title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <button
                                                @click="deleteModalOpen = true; deleteUrl = '{{ route('products.destroy', $product->id) }}'; productName = '{{ $product->name }}'"
                                                type="button"
                                                class="p-2 bg-red-50 hover:bg-red-600 hover:text-white text-red-600 rounded-xl transition-all shadow-xs"
                                                title="Hapus">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <tr>
                                    <td colspan="7" class="py-16 text-center text-gray-400">
                                        <div class="flex flex-col items-center justify-center space-y-2">
                                            ...
                                            <p class="text-xs text-gray-400">Silakan tambahkan produk baru melalui tombol di
                                                atas.</p>
                                        </div>
                                    </td>
                                </tr>
        ...
        <div x-show="deleteModalOpen"
            ...
            <div @click.away="deleteModalOpen = false"
                ...
                <div
                    class="w-14 h-14 rounded-2xl bg-red-50 text-red-600 flex items-center justify-center mx-auto border border-red-100">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Yakin ingin menghapus <span class="font-bold text-gray-800"
                            x-text="productName"></span>? Data yang dihapus tidak bisa dikembalikan.</p>
                </div>
                <div class="flex items-center gap-2 pt-2">
                    <button @click="deleteModalOpen = false" type="button"
                        class="flex-1 py-2.5 rounded-xl border border-gray-200 text-xs font-bold text-gray-700 hover:bg-gray-100 transition">Batal</button>
                    <form :action="deleteUrl" method="POST" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="w-full py-2.5 rounded-xl bg-red-600 text-white text-xs font-bold hover:bg-red-700 transition shadow-md shadow-red-600/20">Ya,
                            Hapus</button>
                    </form>
                </div>
            </div>
