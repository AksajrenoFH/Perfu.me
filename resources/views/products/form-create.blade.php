...
<div class="{{ request('drawer') ? 'py-6' : 'py-12' }} bg-[#F8F9FA] min-h-screen">
    <div class="{{ request('drawer') ? 'max-w-full' : 'max-w-4xl' }} mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            @if(request('drawer'))
                <!-- Tombol Tutup untuk Drawer -->
                <button type="button" onclick="window.parent.postMessage('close-drawer', '*')" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:text-red-600 transition shadow-sm">
                    ✕ Tutup
                </button>
            @else
