    ...
    <div x-data="{ 
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-4 rounded-3xl shadow-xs border border-gray-100">
                <form action="{{ route('reviews.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    ...
                    <div class="md:col-span-5">
                        ...
                        <div class="relative">
                            ...
                            <input id="review-search" type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama reviewer, komentar, atau produk..."
                    ...
                    <div class="md:col-span-3 flex gap-2">
                        ...
                        @if(request('rating') || request('search'))
                            <a href="{{ route('reviews.index') }}" class="px-4 py-2.5 rounded-2xl bg-gray-100 text-xs font-bold text-gray-600 hover:bg-gray-200 transition text-center flex items-center justify-center">Reset</a>
                        @endif
                    </div>
                </form>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-100 text-gray-700">
                            @forelse ($reviews as $index => $review)
                                <tr class="hover:bg-gray-50/70 transition-colors">
                                    <td class="py-4 px-6 text-center font-bold text-gray-400">{{ $reviews->firstItem() + $index }}</td>
                                    
                                    <td class="py-4 px-6">
                                        <div class="font-bold text-gray-900 text-sm tracking-tight">{{ $review->product->name ?? 'Produk Telah Dihapus' }}</div>
                                        @if(isset($review->product) && $review->product->category)
                                            <div class="text-[11px] text-gray-400">{{ $review->product->category }}</div>
                                        @endif
                                    </td>
                                    
                                    <td class="py-4 px-6">
                                        <div class="font-bold text-gray-900 text-sm">{{ $review->user_name }}</div>
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-right">
                                        <div class="flex justify-end items-center gap-3">
                                            <button type="button" @click="openDrawer('{{ route('reviews.show', ['review' => $review, 'drawer' => 1]) }}', 'Detail Ulasan')"
                                            ...
                                            <button type="button" @click="openDrawer('{{ route('reviews.edit', ['review' => $review, 'drawer' => 1]) }}', 'Edit Ulasan')"
                                                title="Edit" class="text-[#b58d17] hover:text-[#8f6e12] transition-colors">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125" />
                                                </svg>
                                            </button>
                                            <button type="button" @click="deleteModalOpen = true; deleteUrl = '{{ route('reviews.destroy', $review->id) }}'; reviewerName = '{{ $review->user_name }}'"
