...
<x-app-layout>
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="POST" action="{{ route('orders.store') }}" class="bg-white border border-gray-100 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
                @csrf
...
<body class="bg-[#F8F9FA] p-6 antialiased">
    <form method="POST" action="{{ route('orders.store', ['drawer' => 1]) }}" class="bg-white border border-gray-100 rounded-3xl p-6 shadow-xs space-y-6">
        @csrf
        @include('orders._form')
        <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
