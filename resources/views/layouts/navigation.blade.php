...
<div x-data="{ mobileOpen: false, isCollapsed: false, openDropdown: {{ request()->routeIs('products.*') ? 'true' : 'false' }}, brandDropdown: {{ request()->routeIs('brands.*') ? 'true' : 'false' }} }">
    ...
    <nav :class="{
        ...
        <div class="flex items-center justify-center h-20 border-b border-gray-50 shrink-0 group overflow-hidden">
            <a href="{{ route('dashboard') }}" class="flex items-center transition-transform duration-300 ease-in-out group-hover:scale-105">
                <!-- Logo Full -->
        ...
        <div class="flex-1 py-8 space-y-3 overflow-y-auto overflow-x-hidden" :class="isCollapsed ? 'px-3' : 'px-4'">
            ...
            <div class="space-y-1">
                <div class="group relative flex items-center justify-between py-3.5 text-sm font-semibold rounded-xl transition-all duration-300 ease-in-out
                    ...
                    <a href="{{ route('products.index') }}" class="flex items-center flex-1 min-w-0 pr-2">
                ...
                <div x-show="openDropdown && !isCollapsed" x-transition.origin.top.duration.300ms class="pl-11 pr-2 space-y-1.5 pt-1" style="display: none;">
                    <a href="{{ route('products.index', ['type' => 'Original']) }}" class="block px-3 py-2 text-xs font-semibold rounded-lg transition-colors {{ request('type') == 'Original' ? 'text-[#D4AF37] bg-gray-100 font-bold' : 'text-gray-500 hover:text-black hover:bg-gray-50' }}">
                        • Produk Original
                    </a>
                    <a href="{{ route('products.index', ['type' => 'Refill']) }}" class="block px-3 py-2 text-xs font-semibold rounded-lg transition-colors {{ request('type') == 'Refill' ? 'text-[#D4AF37] bg-gray-100 font-bold' : 'text-gray-500 hover:text-black hover:bg-gray-50' }}">
                        • Parfum Refill
                    </a>
                </div>
            </div>
        ...
        <div class="p-4 border-t border-gray-100 shrink-0 bg-white" :class="isCollapsed ? 'px-2' : 'px-4'">
            <x-dropdown align="top" width="48">
                <x-slot name="trigger">
                    <button class="group flex items-center w-full py-3 text-sm font-medium text-left text-gray-700 bg-white rounded-xl hover:bg-gray-50 border border-transparent transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/50"
                        ...
                        <div x-show="!isCollapsed" x-transition.opacity.duration.300ms class="flex-1 truncate ml-3" style="display: none;">
                            <div class="font-bold text-gray-900 group-hover:text-black transition-colors">{{ Auth::user()->name }}</div>
                            <div class="text-xs text-gray-500 truncate transition-colors group-hover:text-gray-600">{{ Auth::user()->email }}</div>
                        </div>
                ...
                <x-slot name="content">
                    <div class="py-1">
                        <x-dropdown-link :href="route('profile.edit')" class="transition-colors hover:text-[#D4AF37] hover:bg-gray-50/50">
                            {{ __('Pengaturan Profil') }}
                        </x-dropdown-link>
                        <div class="border-t border-gray-100 my-1"></div>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <x-dropdown-link :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();" class="text-red-600 hover:text-red-700 hover:bg-red-50 transition-colors">
                                {{ __('Keluar Akun') }}
                            </x-dropdown-link>
                        </form>
                    </div>
    ...
    <div class="lg:hidden fixed top-0 w-full flex items-center justify-between h-16 px-4 bg-white/80 backdrop-blur-md border-b border-gray-100 z-30 shadow-sm transition-all duration-300">
        <a href="{{ route('dashboard') }}" class="text-xl font-extrabold text-black">
